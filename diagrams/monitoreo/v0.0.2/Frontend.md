...
| Término | Es | NO es |
|---|---|---|
| **Orden de transporte** (`transport_order`) | Documento de despacho **y** la salida física: un camión con su chofer saliendo del almacén con UNA carga. Nace en nuestro microservicio | Un documento de SAP · el camión (un camión hace N salidas por día) |
...
- Un **camión hace N salidas por día**, cada una con su propia carga y su propia orden.
- **Una orden = una carga = una salida física.** Al anularse `trips`, la orden dejó de ser solo el
  papel: lleva `truck_id`, chofer y `departure_date`. El chofer no intercala: sale, entrega toda la
  orden, vuelve al almacén, recarga y recién ahí arranca la siguiente. Por eso el mismo camión y el
  mismo chofer se repiten en el listado con horarios de salida distintos.
...
| Camión | `trucks.plate` vía `transport_order.truck_id` |
| Chofer | `transport_order.name_driver_employee` |
| Estado de la salida | `transport_order.transport_status` — el heredado de `trips` (PENDING / EN_RUTA / FINALIZADO). **No** es `transport_order.status`, que es el estado del documento (`orderStatus`: DISPATCHED…) |
...
| Salida | `transport_order.departure_date` |

Camión, chofer, estado y salida exigían antes pasar por `trips`; ahora son columnas de la propia
orden. De la identidad de la fila, lo único que sigue saliendo de otra tabla es la placa (`trucks`).
...
**Invariante 2 — un camión, una carga en la calle.** Las cargas de un camión son SECUENCIALES: sale,
reparte, vuelve, recarga y recién ahí sale de nuevo. De ahí se siguen dos reglas que el listado y el
mapa tienen que respetar juntos:

1. **Nunca dos cargas de la misma placa "En ruta" a la vez.** Un camión no está en dos lugares. Si
   una placa se repite en el listado, a lo sumo UNA de sus filas está en ruta.
2. **Los estados son monótonos en el tiempo.** Ordenadas por salida: primero las finalizadas,
   después a lo sumo una en ruta, después las pendientes. Una carga de las 12:00 no puede figurar
   terminada mientras la de las 08:00 sigue afuera.

El costo de romperlo no es cosmético: el mapa dibuja **un pin por carga en ruta**, así que dos filas
en ruta de la misma placa son dos camiones en pantalla —en posiciones distintas— para un solo camión
real. `monitoreo-data.ts` sortea **por camión** cuál de sus cargas está en curso y deduce el resto,
justamente para no poder generar ese estado.
...
`transport_status`. NO lleva `delivery_started` / `delivery_closed`: la tabla no muestra paradas, muestra el
...
| Posición del camión | **DynamoDB** `truck_tracking`: ítem ACTUAL, o la última TRAZA (`Query PK=ORDER#{transportOrderId}`, `ScanIndexForward=false`, `Limit=1`) |
...
| **Posición del camión** | Pin del mapa · encuadre inicial | `truck_tracking`: ítem **ACTUAL** (`FLEET#{distributorId}` / `ORDER#{transportOrderId}`) para el listado, o la **última TRAZA** (`PK=ORDER#{transportOrderId}`, `ScanIndexForward=false`, `Limit=1`) para el detalle. Son el mismo ping escrito dos veces |
...
5. **`transport_order.truck_id` y `driver_employee_id` sin FK declarada.** Venían así del bloque
   `trips` (`UltimaVersion.sql:206-225`, el que se elimina) y el hueco se muda con las columnas a
   `transport_order` (`:281-302`): anular la tabla no lo cierra.
...
7. **Teléfono del chofer** — no existe. Del chofer, la orden solo hereda `driver_employee_id` y
...
12. **`distributorId` acota los cuatro endpoints web y el mock no tenía scope de distribuidora.** Es un
    ...
    `FLEET#{distributorId}`. Que la columna ya estuviera en `transport_order` es lo que deja esa PK
    intacta al anular `trips`: no se perdió ningún dato al eliminar `trips.distributor_id` (`:209`),
    porque era el mismo. Lo que faltaba es que el dato existiera en `src/mockup/monitoreo/`:
